...
export function ErrorState({ message, requestId, onRetry }: ErrorStateProps) {
  return (
    <div className="flex flex-col items-center justify-center rounded-lg border
                    border-red-200 bg-red-50 px-6 py-14 text-center">
      {/* Icon lỗi */}
      ...
      {onRetry && (
        <button
          type="button"
          onClick={onRetry}
          className="mt-5 inline-flex items-center gap-1.5 rounded-md bg-red-600
      ...
      {/* requestId nhỏ ở góc — người dùng báo lỗi cho IT */}
      {requestId && (
        <p className="mt-4 text-xs text-red-400 select-all" title="Mã yêu cầu — cung cấp cho IT khi báo lỗi">
          Mã lỗi: <span className="font-mono">{requestId}</span>
        </p>
      )}
    </div>
  )
